...
import './style.css'
function Pagination({ pagine, setPagine, max }) {
  const [input, setInput] = useState(1);

  const nextPage = () => {
    setInput(parseInt(input) + 1);
    setPagine(parseInt(pagine) + 1);
  };
  ...
  const onKeyDown = (e) => {
    if (e.keyCode === 13) {
      setPagine(parseInt(e.target.value));
  ...
  return (
    <div className="containerPagine">
      <button disabled={pagine === 1 || pagine < 1} onClick={previousPage}>
      ...
      />
      <p>{parseInt(max)}</p>
      <button disabled={pagine === max || pagine > max} onClick={nextPage}>
        <svg
          width="20"
          height="27"
          viewBox="0 0 20 27"
